...
| **Robot phantom-STOPs every second or two in an open room and can't drive** (collision-stop "works" but is far too trigger-happy, worst at ~2–4 m range) | Expected on the Pi 5: gpiozero times the echo in **software** (`pigpio` unavailable), so isolated reads spuriously time out at long range, and each miss correctly biases to `SAFE_STOP`. **Not a wiring fault.** Mitigated by the **bounded "coast last-known" grace** in `GpiozeroUltrasonicSensor`: a single/brief-burst miss re-serves the last VALID distance instead of instantly declaring blindness, bounded by BOTH `ULTRASONIC_GRACE_MS` (default 150 ms) AND `ULTRASONIC_GRACE_MAX_MISSES` (default 2). Tradeoff: ≤ ~150 ms / 2 misses of coasting on the last good reading (≤ ~0.15 m blind travel at 1 m/s) — a bounded risk; **sustained** loss and **near obstacles** still STOP. Tune via `YALP_ULTRASONIC_GRACE_MS` / `YALP_ULTRASONIC_GRACE_MAX_MISSES` (miss budget `0` disables the grace). See `as-built-wiring.md` §2. |
