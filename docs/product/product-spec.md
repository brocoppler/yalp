...
> **DECISION —** v1 is **text-first** for input: you type commands from your laptop. Replies, though, can already be **spoken aloud** — text-to-speech (TTS) is shipped as an optional `--speak` flag (`yalp see --speak`, `yalp agent --speak`, via macOS `say`), so the robot can read its answers out loud now. Voice *input* — speech-to-text (STT), the mic side — is the part still **deferred to a separate track**, bolted on only after the full typed loop works end to end. Treating STT as its own project (mic quality, audio over USB, latency, wake-words) keeps the voice rabbit hole from blocking the core magic. See `roadmap.md` for the full DONE-vs-NEXT.
...
| **Speak its answers aloud** | Reads its replies out loud instead of only printing them — the `speak` tool plus `see`/`agent` narration, enabled with the optional `--speak` flag. | Instant once the answer is ready (local TTS). | Available now via `--speak` (macOS `say`); off by default, so output stays text unless asked. This is voice *output* only — voice *input* (STT) is still deferred (§1). |
